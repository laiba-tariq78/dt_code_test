...
class NotificationService
{
    ...
    public function __construct(LoggerInterface $logger, BookingRepository $bookingRepository, TranslatorService $translatorService, JobService $jobService)
    {
        $this->logger = $logger;
    ...
    public function sendSessionStartReminder($user, $job, $language, $due, $duration)
    {
        ...
        $messageText = $this->getReminderMessage($job, $language, $dueExplode, $duration);

        if ($this->isNeedToSendPush($user->id)) {
            $usersArray = [$user];
            $this->sendPushNotificationToSpecificUsers($usersArray, $job->id, $data, $messageText, $this->bookingRepository->isNeedToDelayPush($user->id));
            $this->logger->info('sendSessionStartRemindNotification', ['job' => $job->id]);
        }
    }

    public function getReminderMessage($job, $language, $dueExplode, $duration)
    {
    ...
    public function sendNotificationToSuitableTranslators($translationJob)
    {
        $translators = User::where('role', 'translator')
            ->where('language', $translationJob->language)
            ->get();

        foreach ($translators as $translator) {
            $translator->notify(new TranslationAssigned($translationJob));
        }
    }
    ...
    public function notifyCustomerOfCancellation($job, $customer)
    {
        $data = ['notification_type' => 'job_cancelled'];
        $language = TeHelper::fetchLanguageFromJobId($job->from_language_id);
        $msg_text = [
    ...
    public function sendNotificationTranslator($job, $data = [], $exclude_user_id)
    {
        ...
        foreach ($users as $oneUser) {
            //what is 1, 2? no body knows better to make enums here
            if ($oneUser->user_type == '2' && $oneUser->status == '1' && $oneUser->id != $exclude_user_id) { // user is translator and he is not disabled
                ...
                $jobs = $this->jobService->getPotentialJobIdsWithUserId($oneUser->id); // get all potential jobs of this user
        ...
        $logger->pushHandler(new StreamHandler(storage_path('logs/push/laravel-' . date('Y-m-d') . '.log'), Logger::DEBUG)); //dont user hardcoded path here make variable
    ...
    {
        $translators = $this->translatorService->getPotentialTranslators($job);
        ...
        //instead of too much if else use ternary operator
    ...
    public function sendPushNotificationToSpecificUsers($users, $job_id, $data, $msg_text, $is_need_delay)
    {
        ...
        //the below 3 lines repeating everywhere try to make common function for optimization
        $logger = new Logger('push_logger');
