...
class TranslatorService
{
    ...
    public function __construct(LoggerInterface $logger, BookingRepository $bookingRepository, NotificationService $notificationService, JobService $jobService)
    {
        $this->logger = $logger;
    ...
    public function handleTranslatorCancellation($job, $translator)
    {
        $response = [];
        if ($job->due->diffInHours(Carbon::now()) > 24) {
            $customer = $job->user()->first();
            if ($customer) {
                $this->notificationService->notifyCustomerOfCancellation($job, $customer);
            }
            $this->jobService->updateJobForNewAssignment($job, $translator);
